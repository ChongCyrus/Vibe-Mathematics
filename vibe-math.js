...
export function apply(ctx) {
  ...
  function parseJson(text) { if (typeof text !== 'string') return undefined; const fence = text.match(/```(?:json)?\s*([\s\S]*?)```/i); const cand = fence ? fence[1] : text; try { return JSON.parse(cand.trim()) } catch (e) {} const start = cand.indexOf('{'); if (start === -1) return undefined; let depth = 0, inStr = false, esc = false; for (let i = start; i < cand.length; i++) { const c = cand[i]; if (inStr) { if (esc) esc = false; else if (c === '\\') esc = true; else if (c === '"') inStr = false; continue } if (c === '"') inStr = true; else if (c === '{') depth++; else if (c === '}') { depth--; if (depth === 0) { try { return JSON.parse(cand.slice(start, i + 1)) } catch (e2) { return undefined } } } } return undefined }
  ...
  async function readCurrentProject() { try { const t = await fs.resolve('current.json', { cwd: vibeRoot() }); const s = await fs.stat(t); if (s === undefined) return 'default'; const txt = await fs.readText(t); const j = safeJson(txt, null); return (j && j.project) ? String(j.project) : 'default' } catch (e) { return 'default' } }
  ...
  // ================= init / control =================
  async function resolveRootAgent(agent) { if (agent) rootAgent = agent; if (!rootAgent) { try { const roots = agents.roots ? agents.roots() : []; if (roots && roots.length > 0) rootAgent = roots[0] } catch (e) {} } return rootAgent }
