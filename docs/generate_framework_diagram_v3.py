...
# 调度层：调度器 + 规划代理（并排）
box(1.0, 8.58, 4.0, 1.08, "#FFE0B2")
txt(3.0, 9.12, "调度器（插件代码）\n唯一物理写者 · 锚点索引/结构性移动\n守硬约束（并发/幂等/已验证不再调度）", fs=10.5)
box(5.4, 8.58, 2.6, 1.08, "#FFE0B2")
txt(6.7, 9.12, "规划代理 Planner\n读状态简报 → 一次安排 N 步计划", fs=10.5)
...
# ── right md knowledge base (x 10.2–17.2) ─────────────────────────────────
box(17.2, 8.58, -6.6, 0.92, "#E1BEE7") if False else None
...
# ── pipeline arrows ──────────────────────────────────────────────────────
arrow(4.5, 10.32, 4.5, 9.66, "vibe_math_* 工具", ly=0.22, ls=9)
arrow(6.7, 9.66, 6.7, 9.12, "", lw=0)  # spacer
...
arrow(4.5, 3.95, 4.5, 2.58, "", lw=0)  # spacer
...
arrow(8.0, 3.42, 10.4, 3.54, "", lw=0)  # method keeper → methods
# Method Keeper 沉淀到方法库
arrow(8.1, 3.42, 10.4, 3.54, "", lw=0)
# planner ↔ 调度器/数据（状态简报）
arrow(6.7, 9.66, 6.7, 9.12, "状态简报", ly=0.20, ls=9, style="<|-|>")
arrow(6.7, 8.58, 6.7, 7.88, "计划 JSON", ly=0.20, ls=9)
# 规划代理读取状态（读 index/对象）+ 上次计划
arrow(13.7, 9.13, 13.7, 9.13, "", lw=0)
