...
**"纯搬动"有测量为证**：每个套件在**改动前的检出**（临时 git worktree）与**改动后的检出**里各跑一遍，
把检出路径、`tests/` 这一段、临时目录、缓存串、随机 id 与耗时归一化之后**逐行比对**
（`_oneoff/layout-invariance.mjs`）：**24/26 输出逐字节相同**，另外 2 个是本轮改过内容的文件
（新增两条链接断言的市场守卫、被改写路径的 v5 套件），**没有一个套件的输出与"套件住在哪里"有关**。
断言条数与归档前逐项一致；README 的 29 条本地链接与全仓 52 条相对链接 0 失效。
（顺带发现审计工具自身的盲区：`git ls-files` 默认会把中文路径 C 风格转义，于是 6 个中文名文档——
含四份 `实现方案.md` 与 `docs/架构图.md`——从来没被链接扫描看过；现已加 `-c core.quotepath=false`，
扫描面从 28 个 md 扩到 35 个、45 条链接扩到 52 条，仍为 0 失效。）
...
**发布前的复核又补了两处**（都是新代码自己审出来的）：

1. **读不到版本时什么都不替换**：manifest 缺失/没有 `version` 字段时不再猜——本次只补回缺失文件、
   并**保留上一次记录的版本号**（这样下次能读到时，日志里的"从哪个版本升级"仍然是对的、备份也落在
   正确的版本目录下）。此前这种情况会被当成"同版本"而静默不动，现在会明确告警说明原因。
2. **备份报告不再误报失败**：同一版本目录下已存在备份时（例如升级被重跑），保留**最早那份**原文，
   并且不再把它报成"备份失败（原文未保留）"——报错必须只说真话。

## 3. 新增与加固的守卫

| 守卫 | 变化 |
|---|---|
| `tests/audit-installer-policy.test.mjs` | **新增**（52 条断言，随包发布）：在临时 DSH_HOME 里驱动真实 `apply()`，用"两个版本的临时包副本"证明基线安装 / 同版本不重写 / 版本一变即替换（且备份原文） / 缺失补回 / 旧 preset 清理 / 读不到版本时不动任何文件，并证明受管清单覆盖四套预设运行时需要的每个文件、备份目录名不会被 DSH 当成 preset |
...
两个新增断言的灵敏度都实测过：`_oneoff/probe-installer-policy.mjs`（对着 2.3.12 的安装器 → **10 条**断言变红，
其中就有"版本一变，被改过的文件必须被替换"）、`_oneoff/probe-market-link-guard.mjs`（README 里塞一条
死链接 / 塞一条"存在但不随包"的链接 → 分别命中对应断言，改回后逐字节还原）。

> **发布前复核还做了一次"引用闭合"审计**（`_oneoff/check-reference-closure.mjs`）：`package.json` 的
> `main`/`exports`/`dsh.bundle.patch`/`files` 全部存在；35 个 markdown 的 52 条相对链接全部可解析；
> 文档与脚本头部的每一条 `node <路径>` 命令行都指向真实文件；代码里"确实写成路径"的字符串字面量
> （排除运行时工作区路径）无一悬空。反向检查顺带把 `docs/COMPAT-AUDIT-ROUND2.md` 纳入了 `files`
> ——README 正文点名让它"详见"，不随包的话 npm 读者就打不开。
