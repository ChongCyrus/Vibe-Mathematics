...
| **组织与领导** | 无领头人，一切靠讨论涌现 | **院士是组织协调中心**：建立全所视图、拆解并**分派**任务、设定优先级、主持会议、督导进度、调配人力——但它的票与别人**等重**（§4.4） |
...
1. **研究所是主体，框架是场地。** 一切任务安排（研究方向、子问题拆分、分工、何时验证、何时结题）由所内成员**群聊/开会**决定。框架只提供**场地与设施**：办公室（持久上下文）、公告栏（规章）、群聊频道、会议室、任务板、成果库、表决箱、档案室。
   ...
   > **"框架不指派"不等于"所内无人组织"。** 框架是场地管理员；**院士是所内的组织者与领导者**。拆解工作、分派任务、设定优先级、督导进度，都是**院士这一成员**的职责（§4.4）——正如现实中研究所的所长统筹全局。这是本方案的一条硬边界：**组织的权力属于院士，中介的权力（且仅是中介）属于框架**，两者不可混淆。
2. **编制与职称是真实的，且院士居组织中心。** 院士、常驻研究员、临时工在**职权、表决权、任期**上确有差别，且差别由机制而非仅由提示词保证（见 §4.3）；院士是**以它为中心**的组织协调枢纽（见 §4.4），但它的**票与别人等重**。
...
| 职位 | 代号 | 数量 | 任期 | 产生方式 |
|---|---|---|---|---|
| **院士**（领头人 / 组织协调中心） | `acad` | 1（可配置 0/1） | 与研究所同寿 | 建所时由框架创建 |
...
2. **转达**：把人的指令转达给研究所（`vibe_v5_message` / `vibe_v5_meeting` / `vibe_v5_hire` / `vibe_v5_fire` / `vibe_v5_pause` / `vibe_v5_resume` / `vibe_v5_stop`）。
...
| 提议议题 / 提议开会 | ✅ | ✅ | ✅（仅提议） | ✅（召集） |
| **召集（真正召开）会议 / 设定议程** | ✅（牵头） | ✅ | ❌ | ✅ |
| **设定全所优先级、决定路线取舍** | ✅（牵头，有决定权） | 建议 | 建议 | ❌ |
| **把工作拆解成任务并分派给任意成员** | ✅（全所调度） | ❌（可提议） | ❌ | ❌ |
| **分派任务给「自己雇的」临时工** | ✅ | ✅ | ❌ | ✅ |
| **督导进度、催办停滞方向、在成员间牵线** | ✅ | — | — | ❌ |
| **调配临时工到不同方向** | ✅（全所） | 仅自己雇的 | ❌ | ✅ |
| 提议对某对象发起验证 | ✅ | ✅ | ✅ | ✅ |
...
### 4.4 院士 = 所内的组织协调中心（"以它为中心"）

**为什么需要它。** 纯自组织在现实中会退化：方向重叠、关键缺口无人认领、成员各说各话、没有全局视图。v4 的真实 run 就出现过"三人各自把问题归约到同一个硬核引理然后集体停滞"。v5 的答案不是让框架去调度（那违反哲学），而是**在研究所内部设一个真实的领导者**，把"全局统筹"变成某个成员的职责。

**院士的七项组织职责：**

| # | 职责 | 机制支撑 |
|---|---|---|
| L1 | **建立并维护全所视图**：谁在做什么、进展如何、瓶颈在哪、哪里有重复/空白 | `vibe_v5_overview`（院士专用，返回编制+任务板+各成员 Progress 摘要+最近群聊） |
| L2 | **拆解与分派**：把原问题拆成值得做的工作，并指派给合适的成员（含临时工） | `vibe_v5_assign {task_id\|新建, to, why, acceptance}`（院士专用） |
| L3 | **设定优先级与取舍**：先做什么、什么该缓、什么该放弃 | 任务的 `priority` 字段（院士可改）+ `vibe_v5_prioritize` |
| L4 | **主持会议、设定议程、收敛结论** | 院士可**直接召开**会议（他人只能提议）；议程由其设定 |
| L5 | **督导进度**：定期检查各成员 Progress/ 与会议发言，催办停滞、纠正偏离 | 院士可 `vibe_v5_nudge {to, why}` 唤醒并点名督办 |
| L6 | **协调资源**：调配临时工、建议增聘/解聘常驻研究员 | `vibe_v5_hire` / `vibe_v5_fire` / 编制提案 |
| L7 | **对外代表**：通过所办向外部汇报与提要求 | `vibe_v5_report` / 留言 |

**同时，四条硬约束防止它变成独裁者**（"仿照现实"，但也不是现实里的坏所长）：

1. **它的票与别人等重。** 院士没有加权票、没有否决权。任何对象要进 `Verified/`，仍须满足 m 票布尔一致（§9）。
2. **它分派的是工作，不是结论。** 院士不能替别人思考，也不能让某个数学断言因为"所长说了"而变正确。
3. **成员可以有理据地反对。** 分派默认应当执行，但成员若认为方向有误、信息过时或有更优路线，**有权先说明理由再决定**；持续分歧走会议裁决（`params.memberMayRejectAssign = true`）。
4. **它不能自我扩张编制。** 增聘/解聘常驻研究员仍需所办/人批准（§10.4）。

> **哲学上的一致性**：这条设计与"框架绝不指派任务"**不冲突**。框架仍然只做中介；**指派者是院士**——一个和研究员一样受 m 票约束的成员。v4 的哲学是"不让框架替成员思考"，v5 保留了它，并补上了 v4 缺的那一环：**所内需要一个负责组织的人**。
...
flowchart TD
    ...
    A ==>|全所视图·拆解·分派·定优先级·督导·调配| R1 & R2 & RN & T1 & T2
    R1 & R2 & RN & T1 & T2 -.->|汇报进展 / 有权据理反对分派| A
    M -->|vibe_v5_*| 框架
    框架 -->|汇报| M
...
```text
你是「<研究所名>」的一名常驻研究员，代号 <你的代号>。本所是一个自组织的合作研究
机构，目标是解决下述研究对象（原问题）：
...
【一、所内编制与你的同事】
  · **院士 <acad>** —— 本所领头人，本所的**组织与协调中心**。它亲自参与研究，同时
    ...
    并代表本所向外部汇报。
    但它的一票与你**等重**，不能单方面定论，也不能让某个结论因为"所长说了"而变正确。
  · **常驻研究员（含你）** —— <列出全部在册常驻研究员代号>。有表决权。
    可自主雇佣/解雇自己的临时工。向院士汇报进展、接受其组织与分派。
  · **临时工** —— <运行时给出在册临时工代号>。由某位研究员或院士为特定任务临时雇入。
    他们可以读、可以想、可以发言、可以写自己的成果库、可以认领或被分派任务，
    但**没有表决权**。
  · **所办（对外接口）** —— 不参与研究、不投票。只有它能代表本所与外部沟通，并把
    外部的指令转达进来。
...
【三、你的资料库、progress 与卡片格式】
  你的资料库根目录：Institutes/<所>/Members/<你的代号>/
...
【四、所内的组织与协调（院士领头）】
  ...
  【重要】分派**不改变求真规则**：院士分派任务、设定优先级，但**不能**因此让任何结论变得
  "正确"。任何对象要进 Verified/，仍然必须满足 m 票布尔一致（见【五】）。院士自己的一票
  与别人**等重**。
...
【八、任务板】
  ...
  · **院士可以直接分派任务**：它可以把任务指派给指定成员（含临时工），并说明理由与
    验收标准。被分派者默认应当执行，但有权先说明理由再决定。
  · **优先级由院士牵头决定**：院士可以调整任务的优先级；你若认为优先级安排有误，说出来。
...
- **首句**改为：「你是「<所名>」的**院士**，本所的领头人与组织协调中心。你不仅亲自做研究，
  还向全所负责组织与推进。」
- **【一】中你的定位句**替换为：「你是本所的组织与协调中心：建立全所视图、拆解并分派工作、
  设定优先级、主持会议、督导进度、调配资源。」
- **【四】整节**替换为下面这一段（以第一人称给出的职责与边界）：

    【四、你的组织职责与边界（院士）】
      作为院士，你对本所的组织与推进负总责：
        ① **建立并维护全所视图**——谁在做什么、进展如何、瓶颈在哪、哪里有重复或空白。
           请定期查看各成员的 Progress/ 与任务板，不要凭印象指挥；
        ② **拆解与分派**——把原问题拆成值得做的工作，分派给合适的成员（含临时工），
           并说清理由与验收标准。选人时优先考虑"谁最适合"，而不只是"谁有空"；
        ③ **设定优先级**——指明先做什么、什么该缓、什么该放弃。资源有限时敢于取舍；
        ④ **主持会议**——召集正式会议、设定议程、维持讨论不跑偏，并把讨论收敛成任务；
        ⑤ **督导进度**——催办停滞的方向、纠正偏离、在成员之间牵线搭桥、避免重复劳动。
           对停滞者不要只是催促，要给出具体的下一步或配对建议；
...
- **【五】中追加**一句：「你享有与所有有表决权者相同的**一票**，不享有更高票权，也不能
  单方面定论。」
- 其余同常驻研究员——**你的资料库与 progress 要求、求真规则、任务板规则完全一样**，
  院士也要认真写自己的 Progress/ 与成果库（并且因为你要统筹全所，你更该保持它们清晰）。

### 6.2 临时工版替换

- **首句**改为：「你是「<所名>」的**临时工**，由 <雇主代号> 雇入，任务期至 <任务说明或"雇主另行通知">。」
- **【一】** 的编制说明改为：「本所有表决权者是院士与常驻研究员；你是临时雇入的协作人员。」
- **【五】表决节**改为：「本所结论由有表决权者（院士与常驻研究员）按 m 票布尔一致决定。
  **你没有表决权**，但你的判断很重要——请把你的意见和理由清楚地告诉雇主或在群聊里说
  出来，供他们参考。若你认为某个结论该被验证，可以提议。」
- **【七】雇佣与解雇**改为：「你可以建议雇主雇佣或解雇他人，但雇佣/解雇的决定权在雇主
  与院士。」
- **【四】组织节**追加一句：「**院士也可以直接分派任务给你**（它统筹全所）。雇主与院士的
  分派都应执行；若你认为分派有误，先说清理由。」
- 其余同常驻研究员——你的资料库与 progress 要求、任务板规则完全一样；临时工同样要写
  自己的 Progress/ 与成果库，**失败的尝试同样值得记录**。
...
院士 vibe_v5_meeting 可直接召开并**设定议程**；其他人（含临时工）只能"提议"，由框架转呈院士/所办
  ※ 会议的**主持与收口**由院士负责：它维持讨论不跑偏，并把讨论收敛成任务（§4.4-L4）。
    若院士缺席或不作为，所办可代为召集（不能让会议无法召开）。
  ↓
框架：
...
照搬 M3 的完整语义（`task-<n>`、`revision` CAS、`blockedBy` DAG 三校验、`ready` 派生、动作矩阵、`writeScopes` 仅告警）。v5 的五点改造：

1. **解雇时回收 owner**（§10.2 步骤①）。
2. **`reassign` 提升为院士/所办专属**（官方是 Lead 专属；v5 的 Lead 是所办，但"协调研究所内部工作"应属院士职权）。
3. **新增 `priority` 字段与 `vibe_v5_prioritize`**：优先级由院士牵头设定（§4.4-L3），表达"全所资源往哪倾斜"。它**不改变**任务的依赖与认领规则，只是一条排序信号。
4. **新增 `vibe_v5_assign`（院士专用）**：把任务**分派**给指定成员——即 §4.4-L2 的机制化。实现上等价于"以院士身份对任意 active 成员执行 reassign + 自动唤醒被分派者"，但额外要求院士填写 `why`（分派理由）与 `acceptance`（验收标准），二者写入任务详情。被分派者可执行、也可据理反对（`params.memberMayRejectAssign`）——**反对不阻塞执行**，但框架会把反对意见连同理由**广播给院士与全所**，避免"秘密抗命"。
5. **人可读镜像**：每次变更后重建 `Shared/TaskBoard.md`（只写不读，最坏情况短暂滞后，权威源仍是投影）。

**认领/被分派即唤醒**：成员 `claim` 或被 `assign` 后，框架按 M1 唤醒它开工；但**暂停期不唤醒**（照搬 v4 §29-T36 的门控），恢复后自然接续。

**分派只承载"工作"，不承载"真理"**：任务板上的 owner/priority 与 `Verified/` 完全无关；任何结论仍须走 §9 的 m 票布尔裁定。
...
| `vibe_v5_meeting` | `{ agenda, kind, target? }` | 开会：**院士/所办直接召开**；其他成员（含临时工）调用同一工具只会**提议**（框架转呈院士/所办）。验证进行中会自动暂存。 |
...
| 工具 | 参数 | 说明 |
|---|---|---|
| `vibe_v5_overview` | `{}` | **全所视图**：编制 + 任务板 + 各成员 Progress 摘要 + 最近群聊 + 停滞告警（L1） |
| `vibe_v5_assign` | `{ task_id?, subject?, description?, to, why, acceptance, priority? }` | **分派任务**给指定成员（含临时工），须写明理由与验收标准（L2） |
...
| `academicianLeads` | `true` | 是否启用院士的组织/分派职权（L1–L7）。`false` 则退化为 v4 式纯自组织，只有所办能协调 |
| `memberMayRejectAssign` | `true` | 成员可否据理反对院士的分派（反对**不阻塞**，但理由会广播给院士与全所） |
...
### P2 · 编制与生命周期
建所（1 院士 + N 研究员，各自 brainstorm 出方向）；`Member` 记录与不可变身份；`provisioning → active|failed` 对账；`vibe_v5_status`/`list`/`wait`；`vibe_v5_configure`/`start`/`pause`/`resume`/`stop`；**院士的 L1 全所视图 `vibe_v5_overview`**（编制 + 各成员 Progress 摘要 + 任务板 + 停滞告警）。

### P3 · 通信
邮箱（pending−delivered、按目标串行、去重、queued 不重发）；`vibe_v5_say`（群聊/私信/表决者）；摘要合批；以根代理身份中继并署名；`vibe_v5_wait`。

### P4 · 任务板
CAS + DAG + 动作矩阵；`priority` 与 `vibe_v5_prioritize`；**`vibe_v5_assign`（院士分派）与 `vibe_v5_nudge`（点名督办）**；人读镜像；认领/被分派唤醒与暂停门控；解雇时回收 owner。

### P5 · 成果库与规章
四个 record 工具（强制 价值程度/动机用途计划/概率）；只读他人库；**章程写入 persona**（三种职位版本，**含 §6【三】完整的资料库路径、progress 定义与用途、卡片格式**）；成员每轮 JSON 协议。
...
- E2E 断言（每条都必须是"能证伪"的）：
  ...
  13. **院士能分派任务**给任意在册成员（含他人雇的临时工），被分派者被唤醒；非院士调用 `vibe_v5_assign` 返回 `V5_NOT_ACADEMICIAN`；
  14. 成员**据理反对**分派后：反对理由被广播给院士与全所（不静默丢弃）；
  15. **院士不能借分派让任何对象进入 `Verified/`**（分派与裁定彻底解耦），且院士的票与研究员**等重**（同一 m 门槛）。
- **敏感性验证**：对 3、4、5、7、13、15 各写一个"故意破坏实现"的探针，确认测试**真的会红**（延续 v4 审计中"用探针证明测试有效"的做法）。
...
| Q7 | 院士的分派强度：成员据理反对后应**照常执行**，还是**暂挂待裁决**？ | 照常执行 + 广播反对理由（避免进度被任意反对卡死）。若你希望更强制衡，可改为"反对即暂挂该任务并由会议裁决" |
...
| `<你的代号>` | `acad` / `r-<n>` / `t-<n>` |
| `<原问题完整陈述>` | 研究对象全文 |
| `<列出全部在册常驻研究员代号>` | **入职时**的在册名单；权威的当前名单以**每轮状态块**为准（见下） |
| `<列出在册临时工代号>` | 运行时变化，只在**每轮状态块**给出（不进 persona） |
| `<m>` | 法定票数 |
| `<verdictMaxRounds>` | 辩论轮次上限 |
| `<雇主代号>` / `<任务说明>` | 仅临时工版 |

> **persona 里写什么、不写什么**（`persona` 属于不可变身份字段，照搬官方"sealed 身份"）：
>
> - **写进 persona（永久、随简历存活、压缩后仍在）**：哲学与背景、通用规章、**资料库路径与
>   卡片格式（含 §6【三】那段完整的 `progress` 定义、用途与写法建议）**、表决规则、你的代号
>   与职位、你入职时的在册名单、以及组织与协调规则（含院士的七项组织职责 L1–L7 与四条边界）。
> - **不写进 persona（每轮以极简状态块给出）**：当前在册临时工、当前 `m`、当前任务板摘要与
>   优先级、新到的群聊/私信摘要、你上一轮的结论、这一轮要你做什么。
>
> 这样做的收益有两层：
> 1. **新成员一入职就完整读到规章与 `progress` 的定义**——满足"每个新加入常驻员工需先阅读"，
>    而且读到的是一份**带用途解释**的说明（不只是"有这个文件"，而是"为什么必须写、写了给谁看"）；
> 2. **任何上下文压缩之后规章依然生效**（因为它在 persona，不在对话里）。v4 曾为此写了一套
>    "压缩后重申核心规则"的补丁，并因此产生了"规则重申每轮重复泄漏"的真实缺陷
>    （v4 §24.1-③）；v5 由架构消除，不需要那个补丁。
